...
contract DeployMoodNFTTest is Test {
    ...
    function testConvertSvgToUri() public view {
        string memory expectedURI = HAPPY_URI;
        string
        ...
        string memory recivedURI = deployMoodNFT.svgToImageURI(svg);

        console.log(expectedURI);
        console.log(recivedURI);
        assert(
            keccak256(abi.encodePacked(expectedURI)) ==
                keccak256(abi.encodePacked(recivedURI))
        );
    }
}
